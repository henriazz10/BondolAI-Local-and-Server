...
# In this list, are all the locals models that can run on Ollama
local_models = ['gemma3:1b', 'gemma3:4b', 'gemma3:12b', 'gemma3:27b',
...
def response(prompt, model, historial):
    ...
    try:
        # If the model is in the list "local_models", we interpretate that it runs on Ollama
        if model in local_models:
            for part in client.chat(model=model, messages=historial, stream=True,): # For each part of the response...
                answer_not_thoughts += part['message']['content'] # We add that part to the complete answer variable
                yield part['message']['content'] # We yield the text part to Bondol's frontend
    ...
    except TypeError: # If suddenly the API, or Ollama changes the response format, we will catch the error
        pass
...
try:
    ...
    sender_email = "bondol.artificial.intelligence@gmail.com" # This is the email address that will send the confirmation emails
    sender_password = os.getenv('EMAIL_SENDER_PASSWORD') # This is the password for the sender email, it should be set as an environment variable for security reasons DONT HARDCODE IT ON PRODUCTATION

    # If the variables arent set correctely, we show a warning message
